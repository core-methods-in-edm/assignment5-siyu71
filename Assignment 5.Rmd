...
## Part I
```{r}
D1 <- as.data.frame(intelligent_tutor)
```
...
D2 <- as.data.frame(intelligent_tutor_new)
...
D2
#Because the scores are all "1", the actual advices are all "no action". For the prediction, some of them are "monitor", but no "intervene".
```
